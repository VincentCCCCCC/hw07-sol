...
object hw07 extends JsApp {
  ...
  def compressFold[A](l: List[A]): List[A] = l.foldRight(Nil: List[A]){
    (h, acc) => 
      acc match {
        case Nil => h :: acc
        case h1 :: tl => if (h1 == h) acc else h :: acc
      }
  }
  ...
  def subst(e: Expr, x: String, v: Val): Expr = {
    ...
    e match {
      ...
      case Function(p, ys, eb) => 
        if (p.contains(x) || ys.contains(x)) e else Function(p, ys, substX(eb))
    }
  }
  ...
  def eval(e: Expr): Val = {
    ...
    e match {
      ...
      case BinOp(And, e1, e2) => 
        val v1 = eval(e1)
        if (toBool(v1)) /* EvalAndTrue */ eval(e2) 
        else /* EvalAndFalse */ v1
      
      // EvalOr* 
      case BinOp(Or, e1, e2) =>
        val v1 = eval(e1)
        if (toBool(v1)) /* EvalOrTrue */ v1 
